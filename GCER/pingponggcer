...
#define ARM_SERVO 0
//typedef enum { DOWN = 2047, WIRESHARK = 1860, UP = 1300, IN = 1665} tube_arm_pos_t;
//void tube_arm(tube_arm_pos_t pos) { slow_servo(ARM_SERVO, pos, 3); }

#define GRABBER_SERVO 2
typedef enum { G_OPEN = 0, G_CLOSED = 1280 } grabber_pos_t;
...
int main() {
    ...
    //drive_for(100, 0, 300);
    // Go to tube
    puts("wait for 4 seconds");
    drive_for(0, 0, 4000);
    //tube_arm(DOWN);
    puts("linefollow one sided");
    drive_for(100, 100, 2500);
    drive_for(100, 0, 400);
    line_follow_one_sided_until((seconds() - start > 52), LS_L, RIGHT, 50);
    grabber(G_CLOSED);
    //msleep(10000);
    //drive_for(100, 100, 10000);
    drive_for(-100, 100, 530);
    stop();
